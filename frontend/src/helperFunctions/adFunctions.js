...
export function getUserName() {
    let storageObject = JSON.parse(sessionStorage.getItem(`msal.token.keys.${process.env.REACT_APP_AZ_CLIENT_ID}`));
...
export function isUserAdmin() {
    ...
    let tokenEncoded = JSON.parse(sessionStorage.getItem(tokenName)).secret
    let tokenDecoded = jwtDecode(tokenEncoded);
    let adAdminGrpId = process.env.REACT_APP_AZ_ADMIN_GRP_ID
