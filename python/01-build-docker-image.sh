...
if [ "$BUILD" == 'true' ]; then
    ...
    BUILD_FAILED='false'
    echo "Building image..."
    docker build -t "ec2c-python-job:$VERSION" . && \
    docker tag ec2c-python-job:$VERSION ec2c-python-job:latest || \
    BUILD_FAILED='true'
...
if [[ "$UPLOAD_TO_ECR" == 'true' && "$BUILD_FAILED" == 'false' ]]; then
    ...
    docker tag ec2c-python-job:latest $AWS_ID.dkr.ecr.$AWS_REGION.amazonaws.com/ec2c-python-job:latest
    docker tag ec2c-python-job:$VERSION $AWS_ID.dkr.ecr.$AWS_REGION.amazonaws.com/ec2c-python-job:$VERSION
